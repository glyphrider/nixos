{ inputs, config, pkgs, lib, monitors, ... }:
...
#        "mod CONTROL, XF86TouchpadToggle, exec, toggle-touchpad"
  ...
  # `monitors` (the output names hyprpaper should paint) is a per-host
  # value supplied via home-manager's extraSpecialArgs in flake.nix, since
  # it's tied to each machine's physical display outputs.
